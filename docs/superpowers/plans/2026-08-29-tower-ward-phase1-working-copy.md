...
2. **Placeholders:** none — superseded draft blocks were removed; every code step shows only the final form. **However, see the ERRATA banner at the top of this file: several embedded sketches that survived editing turned out to be buggy and were corrected during execution — the shipped code is canonical, not the sketches.** "Implementer notes" flag gpui-0.2.2 API names that may need graceful degradation (drop styling, never add deps).
